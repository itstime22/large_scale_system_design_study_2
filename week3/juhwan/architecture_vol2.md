...
- **가십 프로토콜(Gossip Protocol)** 사용
  ...
  - 특정 노드의 heartbeat가 일정 시간 동안 갱신되지 않으면 **장애(offline)** 로 간주
...
3. memtable이 가득 차면 디스크에 **SSTable(Sorted-String Table)** 형태로 flush  
   - SSTable은 키 기준으로 정렬된 불변의 파일

#### 6.5.2 읽기 경로
1. 먼저 memtable(메모리)에서 키 검색
2. 없으면 Bloom Filter, 인덱스를 이용해 어느 SSTable에 존재할지 판단
3. 필요한 SSTable에서 데이터 읽기, 여러 SSTable에 걸쳐 있으면 compaction 과정에서 정리
...
### 6.6 운영 인사이트 정리
- **데이터 모델링**: 잘 설계된 파티션 키가 곧 확장성과 핫스팟 방지의 핵심
- **SLA 기반 튜닝**: 지연, RPO/RTO 목표에 따라 `N/R/W`와 CAP 선택(CP vs AP)을 조정
- **장애 처리 전략**: hinted handoff + Merkle Tree 기반 anti-entropy 조합으로 가용성과 일관성을 동시에 추구
- **관측성**: 쿼럼 지연, read/write 실패율, 가십 상태, 힌트 큐(미전송 데이터) 크기 등을 지속 모니터링