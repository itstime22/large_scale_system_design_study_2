...
### 8.6.2 URL 만료 관리
- TTL(Time To Live) 설정
- 만료된 URL 자동 삭제 또는 비활성화

### 8.6.3 커스텀 단축 URL
- 사용자가 원하는 단축 URL 지정 (예: `short.ly/my-page`)
- 충돌 검사 및 예약어 관리 필요
...
### 제7장: 유일 ID 생성기
- **문제**: 분산 환경에서 전역 유일 ID 생성
- **해결**: 스노플레이크 방식 - 64비트를 타임스탬프·데이터센터·서버·시퀀스로 분할
- **핵심**: 시간순 정렬 가능, 초당 10,000개 이상 생성 가능

### 제8장: URL 단축기
- **문제**: 긴 URL을 짧게 변환하여 공유 용이성 향상
- **해결**: Base-62 인코딩으로 숫자 ID를 7자리 문자열로 변환
- **핵심**: 읽기 중심 워크로드이므로 캐싱이 성능의 핵심

---

## 토론 질문 (Discussion Prompts)

1. 스노플레이크 방식에서 시계 동기화가 실패하면 어떤 문제가 발생할까?
2. URL 단축기에서 Base-62 대신 다른 진법(예: Base-36)을 사용하면 어떤 장단점이 있을까?
3. 단축 URL의 보안을 강화하기 위한 추가 방안은 무엇이 있을까?
4. 매일 1억 개의 URL을 생성할 때, 데이터베이스 스토리지 용량은 얼마나 필요할까?
