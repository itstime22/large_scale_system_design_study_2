...
- **HTTP 프로토콜 사용**
    - 송신 클라이언트 → 수신 클라이언트
    - 메세지 전송 용도로 괜찮은 선택임
        - 페이스북 같은 많은 대중적 채팅 프로그램이 초기에 HTTP 사용한다.
...
- **웹소켓 WebSocket**
    - 서버가 클라이언트에게 비동기적으로 메세지를 보낼 때 가장 널리 사용되는 기술
    - 웹소켓 연결은 클라이언트가 시작한다.
...
- 클라이언트에게 가장 적합한 채팅 서버 추천
    ...
    - 사용 가능한 모든 채팅 서버를 등록해두고, 클라이언트가 접속 시도 시 사전에 정한 기준에 따라 최적의 채팅 서버 골라준다
    - 작동 방식
    - 예제
...
- 여러 단말 사이의 메세지 동기화
    ...
    - 새 메세지 간주 기준
        ...
        - 키-값 저장소에 보관된 메세지, messaage_id > cur_max_message_id
